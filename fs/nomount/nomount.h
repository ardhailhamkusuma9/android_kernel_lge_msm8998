...
/*
 * Compat shim: full_name_hash() gained a salt argument in Linux 4.16
 * ("vfs: make the string hashes salted"). This kernel tree (4.4) still
 * uses the old 2-argument signature, so drop the salt on older trees.
 */
#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 16, 0)
#define full_name_hash(salt, name, len) full_name_hash(name, len)
#endif

/*
 * Compat shim: in_compat_syscall() standardised in Linux 4.6.
 * On arm64 4.4 the equivalent is is_compat_task().
 */
...
/*
 * Compat shim: xattr_handler.{get,set} gained a struct inode * parameter
 * in Linux 4.9 (commit c7c7a1a18af4). In 4.4 the inode is not in the
 * signature, so wrap it away.
 */
#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 9, 0)
#define NM_XATTR_INODE_ARG      /* nothing */
#define NM_XATTR_INODE_CALL     /* nothing */
#define nm_vfs_getxattr(dentry, inode, name, buf, sz) \
    vfs_getxattr((dentry), (name), (buf), (sz))
#define nm_vfs_setxattr(dentry, inode, name, buf, sz, flags) \
    vfs_setxattr((dentry), (name), (buf), (sz), (flags))
#define NM_XATTR_ORIG_GET(proxy, dentry, inode, name, buf, sz, flagsval) \
    (proxy)->orig->get((proxy)->orig, (dentry), (name), (buf), (sz))
#define NM_XATTR_ORIG_SET(proxy, dentry, inode, name, buf, sz, flags) \
    (proxy)->orig->set((proxy)->orig, (dentry), (name), (buf), (sz), (flags))
#else
#define NM_XATTR_INODE_ARG      , struct inode *inode
#define NM_XATTR_INODE_CALL     , inode
#define nm_vfs_getxattr(dentry, inode, name, buf, sz) \
    __vfs_getxattr((dentry), (inode), (name), (buf), (sz))
#define nm_vfs_setxattr(dentry, inode, name, buf, sz, flags) \
    __vfs_setxattr(IDMAP_PATH(info->r_path) (dentry), (inode), (name), (buf), (sz), (flags))
#define NM_XATTR_ORIG_GET(proxy, dentry, inode, name, buf, sz, flagsval) \
    (proxy)->orig->get((proxy)->orig, (dentry), (inode), (name), (buf), (sz) flagsval)
#define NM_XATTR_ORIG_SET(proxy, dentry, inode, name, buf, sz, flags) \
    (proxy)->orig->set((proxy)->orig, IDMAP_CALL (dentry), (inode), (name), (buf), (sz), (flags))
#endif
...
#endif
/* iterate (non-shared) present in 3.11+, removed in 6.6 */
...
#endif
/* iterate (non-shared) present 3.11–6.6; primary detection path on < 4.7 */
#if LINUX_VERSION_CODE < KERNEL_VERSION(6, 6, 0)
